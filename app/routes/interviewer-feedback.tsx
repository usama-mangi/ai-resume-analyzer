...
export const meta = () => [
  { title: "Resumind | Interviewer Feedback" },
        ...
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {[1,2,3,4,5].map(i => (
                <button key={i} type="button" onClick={() => setFormData(p => ({ ...p, rating: i }))} className={cn("w-12 h-12 rounded-lg font-bold text-lg transition-colors", formData.rating === i ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200")}>{i}</button>
              ))}
            </div>
          </div>
