...
const SOURCE_STYLES: Record<string, { bg: string; text: string; dot: string }> = {
  ...
  indeed: { bg: "bg-indigo-50", text: "text-indigo-700", dot: "bg-indigo-500" },
  glassdoor: { bg: "bg-green-50", text: "text-green-700", dot: "bg-green-500" },
  jsearch: { bg: "bg-violet-50", text: "text-violet-700", dot: "bg-violet-500" },
    ...
    <PageShell maxWidth="xl">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-3 flex-wrap">
          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold uppercase tracking-wide ${sourceStyle.bg} ${sourceStyle.text}`}>
            <span className={`w-1.5 h-1.5 rounded-full ${sourceStyle.dot}`} />
            {job.source}
          </span>
          {job.isBookmarked && <span className="text-amber-400 text-lg">★</span>}
          {job.appliedAt && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-green-50 text-green-700">Applied</span>
          )}
          {skillMatch && (
            <div className="flex items-center gap-2 ml-auto">
              <ScoreBadge score={skillMatch.percentage} size="sm" />
        ...
        <p className="text-lg font-medium text-primary-600">{job.companyName}</p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            {job.jobType && (
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                ...
                <p className="text-sm font-medium text-gray-900 capitalize">{job.jobType}</p>
              </div>
            )}
            {(job.workArrangement || job.remoteType) && (
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Work Style</p>
                <p className="text-sm font-medium text-gray-900 capitalize">{job.workArrangement || job.remoteType}</p>
              </div>
            )}
            {job.seniorityLevel && (
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Seniority</p>
                <p className="text-sm font-medium text-gray-900 capitalize">{job.seniorityLevel}</p>
              </div>
            )}
            {job.salaryMin && (
              <div className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            {job.requiredExperienceYears != null && (
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Experience</p>
                <p className="text-sm font-medium text-gray-900">{job.requiredExperienceYears}+ years</p>
              </div>
            )}
            {job.jobFunction && (
              <div className="bg-white rounded-xl border border-gray-200 p-4">
                <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Function</p>
                <p className="text-sm font-medium text-gray-900 capitalize">{job.jobFunction}</p>
              </div>
            )}
          </div>
          ...
          {skillMatch && skillMatch.percentage > 0 && (
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Skill Match Overview</h2>
              <div className="flex items-center gap-4 mb-4">
                <ScoreBadge score={skillMatch.percentage} size="lg" />
                <div>
                  <p className="text-sm font-medium text-gray-900">{skillMatch.matchedSkills.length} of {skillMatch.matchedSkills.length + skillMatch.missingSkills.length} required skills matched</p>
                  <p className="text-xs text-gray-500">Based on your most recent resume</p>
                </div>
              </div>
              {skillMatch.matchedSkills.length > 0 && (
                <div className="mb-3">
                  <p className="text-[11px] font-semibold text-success uppercase tracking-wider mb-2">Matched Skills</p>
              ...
              {skillMatch.missingSkills.length > 0 && (
                <div>
                  <p className="text-[11px] font-semibold text-orange-600 uppercase tracking-wider mb-2">Missing Skills</p>
                  <div className="flex flex-wrap gap-1.5">
                    {skillMatch.missingSkills.map((s) => (
                      <span key={s} className="px-2 py-0.5 bg-orange-50 text-orange-700 rounded-full text-xs font-medium">{s}</span>
                    ))}
                  </div>
                </div>
              )}
          ...
          {job.requiredTechnologies?.length > 0 && (
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Required Technologies</h2>
              <div className="flex flex-wrap gap-1.5">
                {job.requiredTechnologies.map((tech: string) => (
                  <span key={tech} className="px-2.5 py-1 bg-green-50 text-green-700 rounded-full text-xs font-medium">{tech}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {job.preferredTechnologies?.length > 0 && (
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Preferred Technologies</h2>
              <div className="flex flex-wrap gap-1.5">
                {job.preferredTechnologies.map((tech: string) => (
                  <span key={tech} className="px-2.5 py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-medium">{tech}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {job.softSkills?.length > 0 && (
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Soft Skills</h2>
              <div className="flex flex-wrap gap-1.5">
                {job.softSkills.map((skill: string) => (
                  <span key={skill} className="px-2.5 py-1 bg-gray-100 text-gray-700 rounded-full text-xs font-medium">{skill}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {job.benefitsExtended?.length > 0 && (
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Benefits</h2>
              <div className="flex flex-wrap gap-1.5">
                {job.benefitsExtended.map((benefit: string) => (
                  <span key={benefit} className="px-2.5 py-1 bg-amber-50 text-amber-700 rounded-full text-xs font-medium">{benefit}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {(job.educationRequired || job.visaSponsorship != null || job.relocationRequired != null || job.relocationAssistance != null || job.hasManagementResponsibilities != null || job.aiMlInvolved != null || job.industry || job.contractDuration || job.startDate) && (
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Additional Information</h2>
              <div className="grid grid-cols-2 gap-3 text-sm">
                {job.educationRequired && (
                  <div><span className="text-gray-500">Education:</span> <span className="font-medium text-gray-900">{job.educationRequired}</span></div>
                )}
                {job.industry && (
                  <div><span className="text-gray-500">Industry:</span> <span className="font-medium text-gray-900">{job.industry}</span></div>
                )}
                {job.contractDuration && (
                  <div><span className="text-gray-500">Contract:</span> <span className="font-medium text-gray-900">{job.contractDuration}</span></div>
                )}
                {job.startDate && (
                  <div><span className="text-gray-500">Start Date:</span> <span className="font-medium text-gray-900">{job.startDate}</span></div>
                )}
                {job.visaSponsorship != null && (
                  <div><span className="text-gray-500">Visa Sponsorship:</span> <span className={`font-medium ${job.visaSponsorship ? 'text-green-600' : 'text-gray-900'}`}>{job.visaSponsorship ? 'Yes' : 'No'}</span></div>
                )}
                {job.relocationRequired != null && (
                  <div><span className="text-gray-500">Relocation Required:</span> <span className="font-medium text-gray-900">{job.relocationRequired ? 'Yes' : 'No'}</span></div>
                )}
                {job.relocationAssistance != null && (
                  <div><span className="text-gray-500">Relocation Assistance:</span> <span className="font-medium text-gray-900">{job.relocationAssistance ? 'Yes' : 'No'}</span></div>
                )}
                {job.hasManagementResponsibilities != null && (
                  <div><span className="text-gray-500">Management:</span> <span className="font-medium text-gray-900">{job.hasManagementResponsibilities ? 'Yes' : 'No'}</span></div>
                )}
                {job.aiMlInvolved != null && (
                  <div><span className="text-gray-500">AI/ML Involved:</span> <span className="font-medium text-gray-900">{job.aiMlInvolved ? 'Yes' : 'No'}</span></div>
                )}
              </div>
            </div>
          )}
          ...
          {job.description && (
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Description</h2>
          ...
          {job.requirements && (
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Requirements</h2>
